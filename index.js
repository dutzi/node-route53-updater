      ...
      route53.listHostedZones(
        ...
        function (err, res) {
          if (err) {
            ...
            hostedZones = hostedZones.concat(
              underscore.filter(res.HostedZones, function (hostedZone) {
    ...
    function (err) {
      if (err) {
        ...
        if (hostedZones.length > 1) {
          cb(
            new Error('hostedZoneName not unique. Use hostedZoneId parameter.')
          );
  ...
  route53.listResourceRecordSets(
    ...
    function (err, res) {
      if (err) {
        ...
        cb(
          undefined,
          underscore.find(res.ResourceRecordSets, function (recordSet) {
  ...
  if (action === 'CREATE' || action === 'UPDATE') {
    ...
    if (params.iface) {
      ...
      if (iface === undefined) {
        ...
        var ipv4 = underscore.find(iface, function (binding) {
    ...
    } else if (params['public-ip']) {
      publicIp.v4().then((ipAddress) => {
        if (!ipAddress) {
          cb(new Error('could not resolve public ip'));
        } else {
          issueUpdate(null, ipAddress);
        }
      });
    } else {
      var mds = new AWS.MetadataService();
      mds.request(
        '/latest/meta-data/' + (params.metadata || 'public-hostname'),
        issueUpdate
      );
    }
